...
import shutil
from mapper.core.settings_manager import get_version, reset_settings as core_reset_settings
...
def test_reset_settings():
    with patch('mapper.core.settings_manager.config_reset_settings') as mock_reset:
        core_reset_settings()
        mock_reset.assert_called_once()

def test_save_user_settings(temp_config_file):
    settings = {
        'output': 'custom_.map',
        'ignore': '.customignore',
        'omit': '.customomit',
...
def test_load_user_settings(temp_config_file):
    settings = {
        'output': 'custom_.map',
        'ignore': '.customignore',
        'omit': '.customomit',
...
def test_load_user_settings_default(temp_config_file):
...
def test_save_user_settings_with_missing_keys(temp_config_file):
    settings = {
        'output': 'custom_.map'
    }
    save_user_settings(settings)
...
def test_load_user_settings_with_partial_settings(temp_config_file):
    partial_settings = {
        'output': 'custom_.map',
        'verbose': True
    }
...
def test_load_user_settings_with_invalid_json(temp_config_file):
...
def test_reset_settings_removes_config_file(temp_config_file):
    with open(temp_config_file, 'w') as f:
        f.write('{}')
...
def test_save_user_settings_overwrites_existing_file(temp_config_file):
    initial_settings = {
        'output': 'initial_.map'
    }
    updated_settings = {
...
def test_save_user_settings_with_invalid_data(temp_config_file):
    settings = {
        'output': 'custom_.map',
...
def test_load_user_settings_with_incorrect_data_types(temp_config_file):
    incorrect_settings = {
        'output': 123,
        'ignore_hidden': 'yes'
    }
...
def test_load_user_settings_with_unreadable_file(temp_config_file):
    with open(temp_config_file, 'w') as f:
        f.write('{}')
...
def test_load_user_settings_when_config_file_is_directory(monkeypatch):
    with tempfile.TemporaryDirectory(dir=os.getcwd()) as tmpdir:
        config_dir = os.path.join(tmpdir, 'test_dir')
...
def test_load_user_settings_with_additional_unknown_keys(temp_config_file):
...
def test_reset_settings_when_no_config_file_exists(monkeypatch):
    monkeypatch.setattr('mapper.config.CONFIG_FILE', 'nonexistent_config.json')
    reset_settings()
    assert not os.path.exists('nonexistent_config.json')

def test_load_user_settings_with_empty_file(temp_config_file):
    with open(temp_config_file, 'w') as f:
        pass
    loaded_settings = load_user_settings()
    assert loaded_settings == {
...
def test_load_user_settings_with_null_values(temp_config_file):
    settings_with_nulls = {
        'output': None,
        'ignore_hidden': None
    }
...
def test_save_user_settings_handles_os_error(temp_config_file):
    with open(temp_config_file, 'w') as f:
        f.write('{}')
    os.chmod(temp_config_file, 0)
    settings = {
...
def test_load_user_settings_with_symlinked_config_file(monkeypatch):
    real_config_file = temp_config_file = tempfile.NamedTemporaryFile(delete=False).name
    save_user_settings({'output': 'real_.map'})
    symlink_config = 'symlink_config.json'
    try:
        os.symlink(real_config_file, symlink_config)
    except (OSError, NotImplementedError, AttributeError):
        pytest.skip("Symlinks not supported or insufficient privileges on this platform.")
    monkeypatch.setattr('mapper.config.CONFIG_FILE', symlink_config)
    loaded_settings = load_user_settings()
    expected_settings = {
        'output': 'real_.map',
        'ignore': '.mapignore',
    ...
    os.remove(symlink_config)

def test_save_user_settings_with_relative_path(monkeypatch):
    with tempfile.TemporaryDirectory(dir=os.getcwd()) as tmpdir:
        config_file = os.path.join(tmpdir, 'config.json')
        relative_config = os.path.relpath(config_file, start=os.getcwd())
        monkeypatch.setattr('mapper.config.settings.CONFIG_FILE', relative_config)
        settings = {
            'output': 'custom_.map'
        }
        save_user_settings(settings)
